...
export default function CreateWholesalePlanPage() {
  ...
  const loadFeatures = async () => {
    try {
      ...
      console.error("Failed to load features from database:", err);
    } finally {
  ...
  const handleNameChange = (val: string) => {
    ...
    setForm((prev) => ({
      ...prev,
      name: val,
      slug: !slugManuallyEdited ? autoSlug : prev.slug,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!cleanSlug) {
      setError("A valid plan slug is required.");
      return;
    }
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          err.message ||
          "Failed to create wholesale plan. Please check inputs."
      );
      setSubmitting(false);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b pb-5">
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight text-foreground">
              Create Standard Wholesale Plan
            </h1>
          ...
          <div className="flex items-center gap-2.5 border-b pb-3.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                Define the tier name, unique slug identifier, and partner-facing description
              </p>
            </div>
          </div>
